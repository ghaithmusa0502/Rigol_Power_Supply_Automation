...
class DataLogger:
    ...
    def _log_data(self) -> None:
        ...
        try:
            ...
            # *** ADDED: Get operation mode ***
            operation_mode = self.config.get(CONFIG_OPERATION_MODE, MODE_CONSTANT_VOLTAGE)
            ...
            while not self.stop_event.is_set():
                ...
                if current_real_time - last_status_update_time >= 5.0:
                    self.status_queue.put((STATUS_INFO, f"Logging: {points_logged} points collected. Current: {current_measured:.4f} A, Voltage: {voltage_measured:.4f} V")) # Added Voltage
                    last_status_update_time = current_real_time

                # *** MODIFIED: Check threshold based on operation mode ***
                ...
                if elapsed_time >= INITIAL_SETTLING_TIME_S:
                    ...
                    if operation_mode == MODE_CONSTANT_VOLTAGE:
                        value_to_check = abs(current_measured) # Use absolute current
                        unit = "A"
                        limit_type = "Current"
                    else: # MODE_CONSTANT_CURRENT
                        value_to_check = voltage_measured # Use voltage
                        unit = "V"
                        limit_type = "Voltage"

                    if stop_condition == "below":
                        if value_to_check < threshold:
                            self.status_queue.put((STATUS_WARNING, f"Threshold reached: {limit_type} {value_to_check:.4f} {unit} < {threshold:.4f} {unit}. Stopping."))
                            stop_triggered = True
...
class PowerLoggerApp:
    ...
    def __init__(self, root: ThemedTk):
        ...
        self.threshold_label_widget: Optional[ttk.Label] = None
        self.voltage_tooltip: Optional[Tooltip] = None
        self.current_tooltip: Optional[Tooltip] = None
        self.threshold_tooltip: Optional[Tooltip] = None
    ...
    def _setup_control_tab(self) -> None:
        ...
        for mode_text in modes:
            # *** MODIFIED: Added command=self._update_threshold_label ***
            radio = ttk.Radiobutton(op_mode_frame, text=mode_text, variable=self.operation_mode_var, value=mode_text, command=self._update_threshold_label)
        ...
        # *** MODIFIED: Stored tooltip, removed specific text ***
        self.voltage_tooltip = Tooltip(self.voltage_entry, "")
        row += 1
        ...
        # *** MODIFIED: Stored tooltip, removed specific text ***
        self.current_tooltip = Tooltip(self.current_entry, "")
        row += 1

        # Threshold
        # *** MODIFIED: Stored the label ***
        self.threshold_label_widget = ttk.Label(control_frame, text="Stop Threshold (A):")
        self.threshold_label_widget.grid(row=row, column=0, **grid_args)
        ...
        # *** MODIFIED: Stored tooltip ***
        self.threshold_tooltip = Tooltip(self.threshold_entry, "")
        row += 1
        ...
        # *** ADDED: Call the update function to set initial state ***
        self._update_threshold_label()

    def _update_threshold_label(self) -> None:
        """Updates the threshold label and tooltips based on the operation mode."""
        ...
        if not hasattr(self, 'threshold_label_widget') or not self.threshold_label_widget or not self.threshold_label_widget.winfo_exists():
            return
        if not hasattr(self, 'threshold_tooltip') or not self.threshold_tooltip: return
        if not hasattr(self, 'voltage_tooltip') or not self.voltage_tooltip: return
        if not hasattr(self, 'current_tooltip') or not self.current_tooltip: return


        if mode == MODE_CONSTANT_CURRENT:
            self.threshold_label_widget.config(text="Voltage Limit (V):")
            self.threshold_tooltip.text = "In CC mode: Stop logging when Voltage reaches this limit (check 'Stop Condition' in Settings)."
            self.voltage_tooltip.text = "In CC mode: Voltage Limit (OVP)."
            self.current_tooltip.text = "In CC mode: Target Current."
        else: # MODE_CONSTANT_VOLTAGE or default
            self.threshold_label_widget.config(text="Stop Threshold (A):")
            self.threshold_tooltip.text = "In CV mode: Stop logging when Current reaches this limit (check 'Stop Condition' in Settings)."
            self.voltage_tooltip.text = "In CV mode: Target Voltage."
            self.current_tooltip.text = "In CV mode: Current Limit (OCP)."
