...
MIN_UPDATE_INTERVAL_MS = 50 # Increased slightly for stability
...
class DataLogger:
    ...
    def _generate_base_filename(self) -> str:
        ...
        loc = self.config.get(CONFIG_SAVE_LOCATION, ".")
        os.makedirs(loc, exist_ok=True)
        return os.path.join(loc, f"log_V{v}_A{c}_{ts}")
...
class PowerLoggerApp:
    ...
    def _create_plot_toolbar(self):
        if self.canvas and self.plot_tab:
            NavigationToolbar2Tk(self.canvas, self.plot_tab).pack(side=tk.BOTTOM, fill=tk.X)
    ...
    def apply_and_save_config(self) -> bool:
        ...
        try:
            ...
            cfg[CONFIG_SAVE_LOCATION] = self.save_location_var.get().strip()
